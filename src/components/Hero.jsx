...
const KB = {
  ...
  experience: [
    { role: "Web Developer", company: "Luminous Graphics", highlights: ["Built and maintained WordPress & Shopify sites, lifting UX and performance.", "Converted Figma designs into responsive, pixel-perfect interfaces that clients loved.", "Maintained clean docs and improved team workflows."] },
    { role: "Freelance Developer", company: "Independent", highlights: ["Partner with local businesses and startups to design and build custom web solutions — focused on responsive design, fast load times, and real results.", "Implement SEO best practices to drive organic traffic and improve search visibility."] }
  ],
  education: [
...
function getAnswer(q) {
  ...
  if (/\b(who|name|about|tell me|describe)\b/.test(t)) return `${KB.name} is a **${KB.role}** based in ${KB.location}. He partners with founders, startups, and businesses to build products people actually use.`;
  if (/email|contact|reach/.test(t)) return `📧 Reach Kamva at **${KB.email}**`;
  if (/location|based|where|city/.test(t)) return `📍 **${KB.location}**`;
  if (/role|job|title|what does|work/.test(t)) return `Kamva is a **${KB.role}** — he collaborates across the full stack to turn ideas into products that ship.`;
  ...
  if (/hire|available|opportunit|freelanc|collab|work with/.test(t)) return `Kamva takes on freelance projects and loves a good collaboration. Got an idea? Let's chat — **${KB.email}** 🚀`;
  if (/what can you build|what do you build|build/.test(t)) return `Kamva builds **websites**, **web apps**, and **mobile apps** end-to-end — from design to deployment. Got a product idea? He can take it from concept to launch.`;
  if (/how does collab|collaboration work|process|work together/.test(t)) return `Simple: share your idea, we scope it out, agree on deliverables — then Kamva gets it done. No corporate back-and-forth, just clean work and clear comms. Reach out at **${KB.email}** to kick things off.`;
...
const WELCOME = [
  ...
  { id: 3, text: "Or jump to a section using the buttons below ↓ — or just ask me anything 💬" },
];
...
const Hero = () => {
  ...
  return (
    <section className="relative w-full h-screen mx-auto flex flex-col items-center justify-center px-4 pt-20 pb-4 overflow-hidden">
      ...
      >
        {['What can you build?', 'Tell me about your projects', 'How does collaboration work?', 'Let\'s build something together'].map(q => (
          <button
            key={q}
            onClick={() => { setInput(q); }}
